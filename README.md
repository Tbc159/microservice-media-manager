...
deployabile in modo indipendente; **quali** domini siano attivi dipende dall'ambiente ed è
documentato — insieme alle relative API — in [doc/domains-and-api.md](doc/domains-and-api.md).

> Questo README e i documenti di **gestione** in `doc/` (branching, pipeline, sviluppo, governance)
> descrivono il *processo*: sono **universali**, validi su ogni branch a prescindere dai domini
> deployati. Le **funzionalità API** sono specifiche dell'ambiente in cui il dominio è deployato.
...
| [doc/domains-and-api.md](doc/domains-and-api.md) | API per dominio, architettura dei domini con persistenza, implementazioni attuali/future *(specifico dell'ambiente)* |
...
- **Routing `/v0`.** Le path nelle spec sono relative e includono il dominio
  (`/<dominio>`, `/<dominio>/health`); `src/app.py` applica `base_path=/v0` → URL `/v0/<dominio>`.
...
DOMAIN=<dominio> python -m src.app # un solo dominio
...
src/domains/<dominio>/            ← controllers / services (+ repositories/storage/factory se il dominio ha persistenza)
...
deploy/storage/                   ← object storage (presente per i domini che lo richiedono)
...
Secret per ambiente (stessi nomi, valori diversi): `API_HOST`, `API_KEY`, `API_SECRET`; per i
domini con storage in coll/prod anche `MINIO_*` e `STORAGE_*`. Vedi [doc/pipeline.md](doc/pipeline.md).
