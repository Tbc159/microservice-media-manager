...
emit_server_body() {
  ...
  for dom in $PUBLIC_DOMAINS; do
    ...
    echo "        proxy_set_header X-Forwarded-Proto \$proto_esterno;"
    echo "    }"
    echo ""
...

{
  ...
  echo ""
  echo "# Schema ESTERNO: quello con cui il client raggiunge il mondo, non quello con cui noi"
  echo "# riceviamo la richiesta. Davanti a questo proxy puo' essercene un altro che termina il"
  echo "# TLS (l'nginx di sistema) e ci parla in http: sovrascrivendo X-Forwarded-Proto con"
  echo "# \$scheme l'app vedrebbe SEMPRE http e comporrebbe URL assoluti sbagliati — un feed con"
  echo "# <atom:link rel=self> in http finisce cosi' su Podcast Index. Quindi si PROPAGA il"
  echo "# valore ricevuto, e si usa \$scheme solo quando non c'e' nessuno davanti."
  echo "map \$http_x_forwarded_proto \$proto_esterno {"
