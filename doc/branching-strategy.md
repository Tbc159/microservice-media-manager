...
`openapi/shared/`) ridistribuisce **tutti** i domini. Dettaglio della pipeline sul branch `develop`.
...
cambia nel tempo ed è tracciato sul branch `develop` (changelog), non qui: questo documento
descrive il **modello** di branching, valido a prescindere dallo stato di deploy.
